...
export function renderSetupAccountsPage(
  students: any[],
  classesList: string[],
  ...
  const classOptions = classesList.map(c =>
    `<option value="${c}" ${c === selectedClass ? 'selected' : ''}>Kelas ${c}</option>`
  ).join('');
  ...
  if (students.length === 0) {
    ...
    tableRows = students.map((s: any, i: number) => `
      <tr>
  ...
  const teacherRows = (teachers || []).map((t: any, i: number) => {
    ...
    <div class="card border-0 shadow-sm rounded-4 mb-4">
      ...
      <div class="card-body">
        ...
        <form action="/admin/setup-accounts" method="GET" class="row g-2 align-items-center mb-4">
          ...
          ${selectedClass && students.length > 0 && students.some((s: any) => !s.has_account && s.nipd) ? `
