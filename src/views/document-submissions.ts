...
export function renderDocumentSubmissionsPage(
  user: User,
  submissions: any[],
  filters: { status: string; doc_type: string; class_name: string; search: string },
  classes: string[],
  ...
  if (submissions.length === 0) {
    ...
    const rows = submissions.map((sub, i) => {
      ...
      const akteCell = renderDocCell(
        sub.akte_id,
        sub.akte_url,
        sub.akte_status,
        sub.akte_rejection_note,
        sub.akte_uploaded_at,
        sub.akte_submitter_name,
        sub.akte_submitter_username || sub.akte_submitted_by,
        sub.akte_reviewer_name,
        sub.akte_reviewed_at,
        sub.student_name,
        'Akte Kelahiran'
      );

      const kkCell = renderDocCell(
        sub.kk_id,
        sub.kk_url,
        sub.kk_status,
        sub.kk_rejection_note,
        sub.kk_uploaded_at,
        sub.kk_submitter_name,
        sub.kk_submitter_username || sub.kk_submitted_by,
        sub.kk_reviewer_name,
        sub.kk_reviewed_at,
        sub.student_name,
        'Kartu Keluarga'
      );

      const fotoCell = renderDocCell(
        sub.foto_id,
        sub.foto_url,
        sub.foto_status,
        sub.foto_rejection_note,
        sub.foto_uploaded_at,
        sub.foto_submitter_name,
        sub.foto_submitter_username || sub.foto_submitted_by,
        sub.foto_reviewer_name,
        sub.foto_reviewed_at,
        sub.student_name,
        'Foto Profil',
        sub.profile_photo_url  // fallback: ambil langsung dari students.photo_url
      );
