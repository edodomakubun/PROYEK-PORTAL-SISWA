import { MasterClass, Student, StudentClassHistory, User } from '../types';
import { renderLayout } from './layout';

export function renderPromotionPage(
  masterClasses: MasterClass[],
  fromClass: MasterClass,
  selectedToId: number | 'graduated',
  toClass: MasterClass | null,
  isGraduationMode: boolean,
  students: Student[],
  historyLogs: StudentClassHistory[],
  flash: string = ''
): string {
  const fromSelectOptions = masterClasses.map(c =>
    `<option value="${c.id}" ${c.id === fromClass.id ? 'selected' : ''}>${c.name} (Tingkat ${c.level})</option>`
  ).join('');

  const toSelectOptions = [
    `<option value="graduated" ${selectedToId === 'graduated' ? 'selected' : ''}>SISWA LULUSAN (LULUS)</option>`,
    ...masterClasses.map(c =>
      `<option value="${c.id}" ${selectedToId === c.id ? 'selected' : ''}>${c.name} (Tingkat ${c.level})</option>`
    )
  ].join('');
