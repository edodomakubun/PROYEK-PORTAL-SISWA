import { DEFAULT_AVATAR, formatIsoDate } from './helpers';
function renderDocStatusBox(type, status) {
...
export function renderStudentListPage(students, classes, selectedClass, userRole, flashMessage = '') {
    let fatherAlive = 0;
    let fatherAlm = 0;
    let motherAlive = 0;
    let motherAlm = 0;
    students.forEach(s => {
        if (s.is_father_alive === 0)
            fatherAlm++;
        else if (s.father_name)
            fatherAlive++;
        if (s.is_mother_alive === 0)
            motherAlm++;
        else if (s.mother_name)
            motherAlive++;
    });
    return `
  ${flashMessage ? `
...
export function renderStudentDetailPage(student, documents, userRole, flashMessage = '', appSettings = {}, permissions = {}) {
