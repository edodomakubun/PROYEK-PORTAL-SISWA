import { DEFAULT_AVATAR, formatWITDate } from './helpers';
export function renderPriorityStudentsPage(user, priorityList, allStudents, selectedClassFilter = '', flashMessage = '') {
    ...
    priorityList.forEach(p => {
        ...
        if (p.required_photo && !hasPhoto)
            needPhotoCount++;
        if (p.required_kk && !hasKk)
            needKkCount++;
        if (p.required_akte && !hasAkte)
            needAkteCount++;
    });
