...
export function renderGraduatedStudentsPage(students, masterClasses, yearsList, search, yearFilter, userRole) {
    const yearSelectOptions = yearsList.map(y => `<option value="${y}" ${y === yearFilter ? 'selected' : ''}>Tahun Lulus ${y}</option>`).join('');
    const activeClassOptions = masterClasses.map(c => `<option value="${c.id}">${c.name} (Tingkat ${c.level})</option>`).join('');
    let tableRows = '';
...
// ============================================================
export function renderGraduatedStudentDetailPage(student, documents, classHistory, masterClasses, userRole) {
