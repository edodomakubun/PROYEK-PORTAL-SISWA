...
app.get('/audit-log', async (c) => {
    ...
    // Fetch stats
    const totalTodayRes = await db.prepare("SELECT COUNT(*) as cnt FROM portal_audit_logs WHERE action = 'LOGIN_SUCCESS' AND date(created_at) = date('now')").first();
    const failedTodayRes = await db.prepare("SELECT COUNT(*) as cnt FROM portal_audit_logs WHERE action = 'LOGIN_FAILED' AND date(created_at) = date('now')").first();
    const onlineCountRes = await db.prepare("SELECT COUNT(*) as cnt FROM active_sessions WHERE last_seen_at >= datetime('now', '-2 minutes')").first();
    const totalLogsRes = await db.prepare("SELECT COUNT(*) as cnt FROM portal_audit_logs").first();
    const stats = {
...
app.get('/dashboard', async (c) => {
    ...
    try {
        ...
        const totalStudentsRes = await db.prepare(studentCountSql).bind(...countParams).first();
        const totalClassesRes = await db.prepare(classCountSql).bind(...countParams).first();
        let studentsSql = `
      SELECT 
    ...
    `;
        const studentsRes = await db.prepare(studentsSql).bind(...studentParams).all();
...
app.get('/students', async (c) => {
    ...
    try {
        if (restrictClass) {
            ...
            const classesRes = await db.prepare("SELECT DISTINCT class_name FROM students WHERE (status IS NULL OR status = 'active') ORDER BY class_name ASC").all();
    ...
    `;
        ...
        if (restrictClass) {
            ...
            const classesRes = await db.prepare('SELECT DISTINCT class_name FROM students ORDER BY class_name ASC').all();
            classesList = (classesRes.results || []).map(r => r.class_name);
        }
        let sql = `
      SELECT 
...
app.get('/students/my-profile', async (c) => {
    ...
    try {
        const st = await db.prepare('SELECT id FROM students WHERE nipd = ?').bind(user.id).first();
        if (st?.id) {
...
app.get('/priority-students', async (c) => {
    ...
    `;
        ...
        const resAll = params.length > 0 ? await db.prepare(sql).bind(...params).all() : await db.prepare(sql).all();
...
app.post('/api/students/mutate', async (c) => {
    ...
    if (user.role === 'guru' && user.homeroom_class) {
        const targetStudent = await db.prepare('SELECT class_name FROM students WHERE id = ?').bind(studentId).first();
    ...
    try {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).bind(studentId, mutationType, mutationDate, reason, mutationType === 'pindah_sekolah' ? destinationSchool : null, mutationStatus, isAutoApproved ? user.id : null, isAutoApproved ? new Date().toISOString() : null, user.id).run();
...
app.post('/api/admin/mutations/:id/approve', async (c) => {
    ...
    try {
        const mutation = await db.prepare(`SELECT * FROM student_mutations WHERE id = ?`).bind(mutationId).first();
        if (!mutation) {
...
app.post('/api/priority-students/add', async (c) => {
    ...
    `);
        const batch = studentIds.map((id) => stmt.bind(id, notes, requiredPhoto, requiredKk, requiredAkte, String(user.id)));
...
async function checkStudentAccess(c, studentId) {
    const user = c.get('user');
    if (!user)
        return { allowed: false, message: 'Autentikasi diperlukan.' };
    const db = c.env.DB;
...
app.get('/students/:id', async (c) => {
    ...
    if (user.role === 'guru' && user.homeroom_class) {
        const db = c.env.DB;
        const classCheck = await db.prepare('SELECT class_name FROM students WHERE id = ?').bind(studentId).first();
  ...
  `).bind(studentId).all();
    ...
    const permsRes = await db.prepare('SELECT document_type, is_allowed FROM student_document_permissions WHERE student_id = ?').bind(studentId).all();
...
app.post('/api/students', async (c) => {
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    RETURNING id
  `).bind(nipd, nisn, nik, name, class_name, gender, religion, entry_date, birth_place, birth_date).first();
    if (insertStudent?.id) {
  ...
  `).all();
    ...
    sortedClasses.forEach(className => {
        ...
        classStudents.forEach((s, idx) => {
            ...
            if (/^l(aki)?/i.test(genderVal))
                genderVal = 'Laki-Laki';
            else if (/^p(erempuan)?/i.test(genderVal))
                genderVal = 'Perempuan';
            sheetData.push([
                s.id,
                idx + 1,
        ...
        Object.keys(ws).forEach(cellKey => {
            if (cellKey.startsWith('!') || !cellKey.match(/^[C-E]/))
                return;
...
app.post('/api/students/validate-import', async (c) => {
    ...
    `).all();
        ...
        existingStudents.forEach((s) => {
            if (s.nisn)
                byNisn.set(String(s.nisn).trim().toLowerCase(), s);
            if (s.nipd)
                byNipd.set(String(s.nipd).trim().toLowerCase(), s);
        ...
        const results = rows.map((r) => {
            ...
            let matched = null;
            if (r.nisn)
                matched = byNisn.get(String(r.nisn).trim().toLowerCase());
            if (!matched && r.nipd)
                matched = byNipd.get(String(r.nipd).trim().toLowerCase());
            if (!matched)
                matched = byNameClass.get(nameKey);
            if (matched) {
                // Cek perubahan kolom kosong
                const isEmpty = (v) => v === null || v === undefined || String(v).trim() === '';
                const changes = [];
                if (isEmpty(matched.nipd) && r.nipd)
                    changes.push('NIPD');
                if (isEmpty(matched.nisn) && r.nisn)
                    changes.push('NISN');
                if (isEmpty(matched.nik) && r.nik)
                    changes.push('NIK');
                if (isEmpty(matched.class_name) && r.class_name)
                    changes.push('Kelas');
                if (isEmpty(matched.gender) && r.gender)
                    changes.push('Jenis Kelamin');
                if (isEmpty(matched.religion) && r.religion)
                    changes.push('Agama');
                if (isEmpty(matched.entry_date) && r.entry_date)
                    changes.push('Tanggal Masuk');
                if (isEmpty(matched.birth_place) && r.birth_place)
                    changes.push('Tempat Lahir');
                if (isEmpty(matched.birth_date) && r.birth_date)
                    changes.push('Tanggal Lahir');
                if (isEmpty(matched.father_name) && r.father_name)
                    changes.push('Nama Ayah');
                if (isEmpty(matched.mother_name) && r.mother_name)
                    changes.push('Nama Ibu');
                if (changes.length > 0) {
                    countUpdate++;
...
app.post('/api/students/execute-import', async (c) => {
    ...
    try {
        const { rows, validation } = await c.req.json();
        ...
        for (let i = 0; i < rows.length; i++) {
            ...
            if (/^l(aki)?/i.test(gVal))
                gVal = 'Laki-Laki';
            else if (/^p(erempuan)?/i.test(gVal))
                gVal = 'Perempuan';
            if (val.status === 'new') {
            ...
            INSERT INTO students (nipd, nisn, nik, name, class_name, gender, religion, entry_date, birth_place, birth_date)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
          `).bind(r.nipd || null, r.nisn || null, r.nik || null, r.name, r.class_name || '', gVal || null, r.religion || null, r.entry_date || null, r.birth_place || null, r.birth_date || null));
            ...
            `).bind(r.father_name || '', r.mother_name || ''));
                ...
                const studentBinds = [];
                if (val.changes.includes('NIPD')) {
                    studentUpdates.push('nipd = ?');
                    studentBinds.push(r.nipd);
                }
                if (val.changes.includes('NISN')) {
                    studentUpdates.push('nisn = ?');
                    studentBinds.push(r.nisn);
                }
                if (val.changes.includes('NIK')) {
                    studentUpdates.push('nik = ?');
                    studentBinds.push(r.nik);
                }
                if (val.changes.includes('Kelas')) {
                    studentUpdates.push('class_name = ?');
                    studentBinds.push(r.class_name);
                }
                if (val.changes.includes('Jenis Kelamin')) {
                    studentUpdates.push('gender = ?');
                    studentBinds.push(gVal || null);
                }
                if (val.changes.includes('Agama')) {
                    studentUpdates.push('religion = ?');
                    studentBinds.push(r.religion || null);
                }
                if (val.changes.includes('Tanggal Masuk')) {
                    studentUpdates.push('entry_date = ?');
                    studentBinds.push(r.entry_date || null);
                }
                if (val.changes.includes('Tempat Lahir')) {
                    studentUpdates.push('birth_place = ?');
                    studentBinds.push(r.birth_place);
                }
                if (val.changes.includes('Tanggal Lahir')) {
                    studentUpdates.push('birth_date = ?');
                    studentBinds.push(r.birth_date);
                }
                if (studentUpdates.length > 0) {
                    studentBinds.push(studentId);
                    batchStatements.push(db.prepare(`UPDATE students SET ${studentUpdates.join(', ')} WHERE id = ?`).bind(...studentBinds));
                }
                // Update Parent via Upsert
                const parentUpdates = [];
                const parentBinds = [];
                if (val.changes.includes('Nama Ayah')) {
                    parentUpdates.push('father_name = ?');
                    parentBinds.push(r.father_name);
                }
                if (val.changes.includes('Nama Ibu')) {
                    parentUpdates.push('mother_name = ?');
                    parentBinds.push(r.mother_name);
                }
        ...
        const parts = [];
        if (addedCount > 0)
            parts.push(`${addedCount} siswa baru ditambahkan`);
        if (updatedCount > 0)
            parts.push(`${updatedCount} data diperbarui`);
        if (skippedCount > 0)
            parts.push(`${skippedCount} dilewati`);
...
app.post('/api/students/validate-update-by-id', async (c) => {
    ...
    try {
        const { rows, selectedFields } = await c.req.json();
...
app.post('/api/students/execute-update-by-id', async (c) => {
    ...
    try {
        const { rows, selectedFields, validation } = await c.req.json();
        ...
        for (let i = 0; i < rows.length; i++) {
            ...
            if (activeStudentFields.length > 0) {
                ...
                batchStatements.push(db.prepare(`UPDATE students SET ${setClauses.join(', ')} WHERE id = ?`).bind(...binds));
            }
            // Update student_parents table (UPSERT)
            if (activeParentFields.length > 0) {
                const getParentVal = (f) => {
                    let v = r[f];
                    if (f === 'is_father_alive' || f === 'is_mother_alive') {
                        if (typeof v === 'string') {
          ...
          `).bind(studentId, getParentVal('father_name') ?? '', getParentVal('is_father_alive') ?? 1, getParentVal('mother_name') ?? '', getParentVal('is_mother_alive') ?? 1));
            }
            updatedCount++;
        }
        if (batchStatements.length > 0) {
    ...
    `).all();
        ...
        for (const sheetName of wb.SheetNames) {
            ...
            for (let i = 0; i < Math.min(rows.length, 10); i++) {
                ...
                const hasName = rowLower.some((v) => v === 'nama' || v === 'nama lengkap' || v === 'nama siswa');
                if (hasName) {
                    headerIdx = i;
                    rowLower.forEach((val, idx) => {
                        if (val === 'nipd' || val === 'no. induk' || val === 'no induk')
                            colMap['nipd'] = idx;
                        if (val === 'nisn')
                            colMap['nisn'] = idx;
                        if (val === 'nik')
                            colMap['nik'] = idx;
                        if (val === 'nama' || val === 'nama lengkap' || val === 'nama siswa')
                            colMap['name'] = idx;
                        if (val === 'kelas' || val === 'class' || val === 'rombel')
                            colMap['class_name'] = idx;
                        if (val === 'tempat lahir' || val === 'tmp lahir' || val === 'tmp. lahir')
                            colMap['birth_place'] = idx;
                        if (val === 'tanggal lahir' || val === 'tgl lahir' || val === 'tgl. lahir')
                            colMap['birth_date'] = idx;
                        if (val === 'nama ayah' || val === 'ayah')
                            colMap['father_name'] = idx;
                        if (val === 'nama ibu' || val === 'ibu')
                            colMap['mother_name'] = idx;
                    });
                    break;
                }
            }
            if (headerIdx < 0 || !colMap['name'])
                continue;
            // Process data rows
            for (let i = headerIdx + 1; i < rows.length; i++) {
              ...
              `).bind(...valuesToBind).run();
                        ...
                        if (parentChanged) {
                            ...
                            const existingParent = await db.prepare('SELECT id FROM student_parents WHERE student_id = ?').bind(matchedStudent.id).first();
                            if (existingParent) {
                ...
                `).bind(matchedStudent.id, excelFatherName || '', excelMotherName || '').run();
                        ...
                        if (fieldsToUpdate.length > 0 || parentChanged) {
                            ...
                            if (excelNipd && isEmpty(matchedStudent.nipd))
                                matchedStudent.nipd = excelNipd;
                            if (excelNisn && isEmpty(matchedStudent.nisn))
                                matchedStudent.nisn = excelNisn;
                            if (excelNik && isEmpty(matchedStudent.nik))
                                matchedStudent.nik = excelNik;
                            if (excelBirthPlace && isEmpty(matchedStudent.birth_place))
                                matchedStudent.birth_place = excelBirthPlace;
                            if (excelBirthDate && isEmpty(matchedStudent.birth_date))
                                matchedStudent.birth_date = excelBirthDate;
                            if (excelFatherName && isEmpty(matchedStudent.father_name))
                                matchedStudent.father_name = excelFatherName;
                            if (excelMotherName && isEmpty(matchedStudent.mother_name))
                                matchedStudent.mother_name = excelMotherName;
                        }
                        else {
              ...
              VALUES (?, ?, ?, ?, ?, ?, ?)
              RETURNING id
            `).bind(excelNipd, excelNisn, excelNik, name, excelClassName, excelBirthPlace, excelBirthDate).first();
                        if (insertResult?.id) {
                ...
                `).bind(insertResult.id, excelFatherName || '', excelMotherName || '').run();
                            ...
                            if (excelNisn)
                                byNisn.set(excelNisn.toLowerCase(), newStudent);
                            if (excelNipd)
                                byNipd.set(excelNipd.toLowerCase(), newStudent);
        ...
        if (addedCount > 0)
            parts.push(`${addedCount} siswa baru ditambahkan`);
        if (skippedCount > 0)
            parts.push(`${skippedCount} data sudah lengkap/dilewati`);
...
// ----------------------------------------------------
async function isStudentUploadAllowed(db, studentId, docType, settings) {
    let globalKey = '';
    if (docType === 'foto')
        globalKey = 'allow_photo_upload';
    if (docType === 'akte_kelahiran')
        globalKey = 'allow_akte_upload';
    if (docType === 'kartu_keluarga')
        globalKey = 'allow_kk_upload';
...
app.post('/api/students/:id/upload-dual-docs', async (c) => {
    ...
    const body = await c.req.parseBody();
    const akteFile = body['akte_document'];
    const kkFile = body['kk_document'];
    let uploadedCount = 0;
    const db = c.env.DB;
    const bucket = c.env.PORTAL_SISWA_BUCKET;
    const settings = await getAppSettings(db);
...
async function serveR2File(c, key) {
...
app.get('/admin/setup-accounts', async (c) => {
  ...
  `;
    ...
    const teachersRes = await db.prepare('SELECT u.id, u.id as username, t.full_name, u.is_document_reviewer FROM users u LEFT JOIN teacher_profiles t ON u.id = t.user_id WHERE u.role = ?').bind('teacher').all();
...
app.post('/admin/setup-accounts/deactivate', async (c) => {
    ...
    if (user.role === 'guru' && user.homeroom_class) {
        const classCheck = await db.prepare('SELECT class_name FROM students WHERE nipd = ?').bind(nipd).first();
        if (classCheck && classCheck.class_name !== user.homeroom_class) {
...
app.get('/admin/wali-kelas', async (c) => {
    ...
    const classesRes = await db.prepare("SELECT DISTINCT class_name FROM students WHERE class_name IS NOT NULL AND class_name != '' AND (status IS NULL OR status = 'active') ORDER BY class_name ASC").all();
...
app.post('/api/students/:id/documents/delete', async (c) => {
    try {
        ...
        if (user.role === 'guru') {
            let permitted = false;
            if (docType === 'photo' && settings.teacher_delete_photo === '1')
                permitted = true;
            if (docType === 'kartu_keluarga' && settings.teacher_delete_kk === '1')
                permitted = true;
            if (docType === 'akte_kelahiran' && settings.teacher_delete_akte === '1')
                permitted = true;
            if (!permitted) {
                await logAudit(db, {
                    userId: user.id,
        ...
        const student = await db.prepare('SELECT id, name, photo_url FROM students WHERE id = ?').bind(studentId).first();
        if (!student) {
...
app.get('/admin/print-cards/student/:id', async (c) => {
    ...
    const studentId = parseInt(c.req.param('id'), 10);
    if (isNaN(studentId))
        return c.text('ID Siswa tidak valid', 400);
    const student = await db.prepare(`
    SELECT 
      s.id, s.nipd, s.nisn, s.nik, s.name, s.class_name, s.gender, s.photo_url, s.birth_place, s.birth_date, s.status,
      p.father_name, p.mother_name
  ...
  `).bind(studentId).first();
    if (!student)
        return c.text('Data siswa tidak ditemukan.', 404);
    if (user.role === 'guru' && user.homeroom_class) {
        if (student.class_name !== user.homeroom_class) {
    ...
    const classesRes = await db.prepare("SELECT DISTINCT class_name FROM students WHERE class_name IS NOT NULL AND class_name != '' AND (status IS NULL OR status = 'active') ORDER BY class_name ASC").all();
    const classesList = (classesRes.results || []).map(r => r.class_name);
    return c.html(renderPrintCardsPage([student], classesList, student.class_name || ''));
});
// ----------------------------------------------------
...
export function parseAndValidateNik(nik, birthDateDb, genderDb) {
    const errors = [];
...
app.post('/api/students/:id/parse-nik', async (c) => {
    ...
    const student = await db.prepare('SELECT id, nipd, nisn, nik, name, class_name, gender, religion, entry_date, birth_place, birth_date FROM students WHERE id = ?').bind(studentId).first();
    if (!student) {
...
export function computeNextClassName(className) {
...
async function syncMasterClasses(db) {
    try {
        const existingRes = await db.prepare('SELECT id, name, level, next_class_id FROM classes ORDER BY level ASC, name ASC').all();
        const existingClasses = existingRes.results || [];
        const existingNames = new Set(existingClasses.map(c => c.name.toUpperCase().trim()));
        const studentClassesRes = await db.prepare("SELECT DISTINCT class_name FROM students WHERE class_name IS NOT NULL AND class_name != ''").all();
        ...
        const refreshedRes = await db.prepare('SELECT id, name, level, next_class_id FROM classes ORDER BY level ASC, name ASC').all();
...
app.get('/admin/naik-kelas', async (c) => {
    ...
    ORDER BY s.name ASC
  `).bind(fromClass.id, fromClass.name).all();
    const students = studentsRes.results || [];
...
app.post('/api/students/promote', async (c) => {
    ...
    const studentIds = studentIdsRaw.map((v) => parseInt(String(v), 10)).filter((v) => !isNaN(v) && v > 0);
    ...
    let fromClass = await db.prepare('SELECT id, name, level, next_class_id FROM classes WHERE id = ?').bind(fromClassId).first();
    if (!fromClass) {
    ...
    if (!isGraduationMode) {
        if (toClassIdRaw && !isNaN(parseInt(toClassIdRaw, 10)) && parseInt(toClassIdRaw, 10) > 0) {
            ...
            toClass = await db.prepare('SELECT id, name, level, next_class_id FROM classes WHERE id = ?').bind(targetId).first();
        }
        if (!toClass) {
            const nextInfo = computeNextClassName(fromClass.name);
            if (nextInfo.isGraduation) {
                ...
                toClass = await db.prepare('SELECT id, name, level, next_class_id FROM classes WHERE UPPER(TRIM(name)) = UPPER(TRIM(?))').bind(nextInfo.nextClassName).first();
                if (!toClass) {
                    await db.prepare(`
...
app.get('/admin/graduated-students', async (c) => {
    ...
    const yearsRes = await db.prepare("SELECT DISTINCT graduation_year FROM students WHERE status = 'graduated' AND graduation_year IS NOT NULL ORDER BY graduation_year DESC").all();
...
app.post('/api/students/restore-graduated', async (c) => {
    ...
    // 1. Fetch student
    const student = await db.prepare("SELECT id, name, class_id, class_name, status FROM students WHERE id = ? AND status = 'graduated'").bind(studentId).first();
    if (!student) {
        return c.json({ success: false, message: 'Data siswa lulusan tidak ditemukan.' }, 404);
    }
    // 2. Fetch target class
    const targetClass = await db.prepare('SELECT id, name, level FROM classes WHERE id = ?').bind(targetClassId).first();
    if (!targetClass) {
...
app.get('/api/admin/students/:id/permissions', async (c) => {
    ...
    const studentId = parseInt(c.req.param('id'), 10);
    const db = c.env.DB;
    const perms = await db.prepare('SELECT document_type, is_allowed FROM student_document_permissions WHERE student_id = ?').bind(studentId).all();
    let allow_akte = true;
    let allow_kk = true;
    if (perms.results) {
        perms.results.forEach(p => {
            if (p.document_type === 'akte_kelahiran')
                allow_akte = p.is_allowed === 1;
            if (p.document_type === 'kartu_keluarga')
                allow_kk = p.is_allowed === 1;
        });
    }
...
app.post('/api/reviewer/documents/:id/approve', async (c) => {
    ...
    const db = c.env.DB;
    const doc = await db.prepare('SELECT student_id, doc_type, file_url FROM student_documents WHERE id = ?').bind(docId).first();
    await db.prepare(`
...
app.post('/api/reviewer/documents/:id/reject', async (c) => {
    ...
    const db = c.env.DB;
    const doc = await db.prepare('SELECT student_id, doc_type, file_path FROM student_documents WHERE id = ?').bind(docId).first();
    if (doc && doc.file_path) {
        // Delete from R2
        try {
            if (c.env.PORTAL_SISWA_BUCKET) {
                await c.env.PORTAL_SISWA_BUCKET.delete(doc.file_path);
            }
        }
        catch (e) {
...
app.post('/api/reviewer/documents/reject-student/:id', async (c) => {
    ...
    const note = String(body['rejection_note'] || '');
    const db = c.env.DB;
    const docs = await db.prepare('SELECT id, file_path FROM student_documents WHERE student_id = ? AND status = ?').bind(studentId, 'pending').all();
    if (docs.results) {
...
app.get('/document-submissions', async (c) => {
  ...
  `).bind(...params, ...havingParams).first();
  ...
  `;
    ...
    if (restrictClass) {
        ...
        const classesRes = await db.prepare('SELECT DISTINCT class_name FROM students WHERE class_name IS NOT NULL ORDER BY class_name ASC').all();
        classes = (classesRes.results || []).map(r => r.class_name);
    }
    return c.html(renderDocumentSubmissionsPage(user, submissions, { status: statusFilter, doc_type: docTypeFilter, class_name: classNameFilter, search: searchFilter }, classes, { currentPage: page, totalPages, totalItems: total }));
});
export default app;
